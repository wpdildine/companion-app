...
**11) Node map / scene contract** — (1) **TouchZones opacity is scene-owned:** TouchZones.tsx uses only `style.areaPlaneOpacityRules`, `style.areaPlaneOpacityCenter`, `style.areaPlaneOpacityCards`. Both initial JSX materials and the useFrame loop must reference these same three fields. GLSceneZonesStyle contains only these per-zone opacities; getSceneDescription().zones.style is the only source of truth. (2) **Validation matches scene:** validateVizState(state, options?) with options.maxPerCluster (default 8). When validating state and scene is available, pass `{ maxPerCluster: scene.maxPerCluster }`. (3) **getSceneDescription():** Computed on each call; callers store the result on nodeMapRef.current.scene. _options is for future use (paletteId / vizIntensityProfile), not relied on yet. (4) **MAX_PER_CLUSTER:** Fixed cluster size used by buildClusterNodesFromParams and buildLinkEdges; scene.maxPerCluster exposes it. Making max-per-cluster dynamic is a deliberate future change that must update node generation, link topology, scene.maxPerCluster, and consumers—do not change scene.maxPerCluster alone. (5) **Node map / materials:** Spine.tsx MUST NOT define shader code or create materials. All GPU materials live in nodeMap/materials/. Spine.tsx only assigns materials and updates uniforms. Edge shader materials may use R3F \<shaderMaterial\> but must import shader strings from nodeMap/materials/halftone/ and must not embed shader code inline.
