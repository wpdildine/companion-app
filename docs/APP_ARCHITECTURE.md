...
**Play/Act** is a **data-driven, derived** layer documented in [docs/PLAY_ACT_CONTRACT.md](PLAY_ACT_CONTRACT.md). It classifies the current moment into a minimal set of **Acts** (Intake, Evaluate, Clarify, Recover, Respond) from **already-normalized** orchestrator truth and surface-visible arbitration facts. It **does not** replace AgentOrchestrator (lifecycle, request, commit/clear), AgentSurface (arbitration, hold accept/reject per [docs/INTERACTION_CONTRACT.md](INTERACTION_CONTRACT.md)), or VisualizationController / ResultsOverlay (pixels and overlay layout). Optional consumption: presentation may use Act **labels and affordance hints** only in **intersection** with authoritative lifecycle and arbitration. Realization notes: [docs/PLAY_ACT_REALIZATION.md](PLAY_ACT_REALIZATION.md); pure resolver `resolveAgentPlayAct` (`src/app/agent/resolveAgentPlayAct.ts`). **Cycle 6:** semantic channel consumes Play/Act **only** for accessibility label and optional caption (`PLAY_ACT_PHASE_CAPTION_ENABLED` in `AgentSurface`), via `playActPhaseCopy`—no overlay, visualization, or arbitration changes. **Cycle 7:** hardening targets, Stage 2 go/no-go, future-consumer boundaries, and expansion stop rules: [docs/PLAY_ACT_BOUNDARIES.md](PLAY_ACT_BOUNDARIES.md). **Cycle 8:** Stage 2 visible phase caption **on** by default (`PLAY_ACT_PHASE_CAPTION_ENABLED` in `AgentSurface`), same shell and mappers as Cycle 6–7—see [docs/PLAY_ACT_REALIZATION.md](PLAY_ACT_REALIZATION.md) Cycle 8. **Cycle 10:** the same canonical Play/Act accessibility label is applied to the `ResultsOverlay` root container (interpretation only; overlay still owns reveal/dismiss/layout)—see [docs/PLAY_ACT_REALIZATION.md](PLAY_ACT_REALIZATION.md) Cycle 10.
