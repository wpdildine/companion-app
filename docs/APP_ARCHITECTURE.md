...
**Play/Act** is a **data-driven, derived** layer documented in [docs/PLAY_ACT_CONTRACT.md](PLAY_ACT_CONTRACT.md). The legacy resolver `resolveAgentPlayAct` (`src/app/agent/resolveAgentPlayAct.ts`) classifies the current moment into **primaryAct** + **commitVisibilityHint** + **affordance hints** from orchestrator truth (and band intersection for hints only). It **does not** replace AgentOrchestrator (lifecycle, request, commit/clear), AgentSurface (arbitration, hold accept/reject per [docs/INTERACTION_CONTRACT.md](INTERACTION_CONTRACT.md)), or VisualizationController / ResultsOverlay (pixels and overlay layout). **Canonical** semantic-channel **accessibility label** and **visible phase caption** are owned by `semanticChannelCanonicalCopy.ts` (`src/app/agent/semanticChannelCanonicalCopy.ts`) (`getSemanticEvidence` + `resolveActDescriptor` in `AgentSurface`; shared phase table with the resolver). Optional consumption of **affordance hints** remains intersected with authoritative lifecycle and arbitration. Realization notes: [docs/PLAY_ACT_REALIZATION.md](PLAY_ACT_REALIZATION.md). Parity matrix: [docs/semanticChannelCopyParity.matrix.md](semanticChannelCopyParity.matrix.md). **Cycle 6–8:** label/caption wiring in `AgentSurface` (`PLAY_ACT_PHASE_CAPTION_ENABLED`); **Cycle 7:** [docs/PLAY_ACT_BOUNDARIES.md](PLAY_ACT_BOUNDARIES.md). **Cycle 10:** the same canonical accessibility string is applied to the `ResultsOverlay` root—see [docs/PLAY_ACT_REALIZATION.md](PLAY_ACT_REALIZATION.md) Cycle 10.
