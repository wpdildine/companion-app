...
- **src/nodeMap/** — Pure visualization layer (NodeMapCanvas, NodeMapSurface, etc.). components/, interaction/, helpers/, shaders/, types.ts. No app state or theme import; receives injected theme primitives + engine ref.
...
- **src/utils/** — log, validateVizState. Pure or side-effect isolated; no React/theme imports.
