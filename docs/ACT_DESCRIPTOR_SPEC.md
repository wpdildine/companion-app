...
**Related:** [docs/APP_ARCHITECTURE.md](APP_ARCHITECTURE.md) (ownership), [docs/PLAY_ACT_CONTRACT.md](PLAY_ACT_CONTRACT.md) (legacy Play/Act resolver is **compat** for affordances/drift; **canonical** semantic-channel label/caption live in [`semanticChannelCanonicalCopy.ts`](../src/app/agent/semanticChannelCanonicalCopy.ts)), [docs/semanticChannelCopyParity.matrix.md](semanticChannelCopyParity.matrix.md), [src/app/agent/semanticEvidenceTypes.ts](../src/app/agent/semanticEvidenceTypes.ts), [src/app/agent/getSemanticEvidence.ts](../src/app/agent/getSemanticEvidence.ts).
...
**Second adopted consumer (Cycle 2):** Supplementary **accessibility hint** on the semantic channel `ScrollView` ([SemanticChannelView.tsx](../src/screens/voice/SemanticChannelView.tsx)), from `getActDescriptorSemanticChannelHint` in [actDescriptorSemanticChannelHint.ts](../src/app/agent/actDescriptorSemanticChannelHint.ts). **Canonical** semantic-channel **label** and **visible phase caption** are owned by [semanticChannelCanonicalCopy.ts](../src/app/agent/semanticChannelCanonicalCopy.ts) (orchestrator snapshot via `SemanticEvidence.runtime` + optional __DEV__ Act coherence checks); they are **not** Act-driven control surfaces. The hint mapper uses **`identity.family`** and **`semanticSituation.inFlightBucket`** (for `WorkInFlight`) only—no pathways, affordances, or imperatives. **`lifecycle === 'error'`** suppresses the hint so orchestrator error truth matches canonical copy. Toggle: `ACT_DESCRIPTOR_SEMANTIC_CHANNEL_HINT_ENABLED` in `AgentSurface.tsx`.
...
| **Semantic channel a11y hint** | Neutral situation gloss from `identity.family` (+ `inFlightBucket` for `WorkInFlight`); same `getSemanticEvidence` inputs as the shell | Gating; pathways/affordances in copy; replacing **canonical** label/caption (`semanticChannelCanonicalCopy`) |
...
**IMPLEMENTED** — schema, resolver, **dev snapshot** consumer, **semantic channel accessibility hint** (bounded product consumer), and **canonical** semantic-channel label/caption in `semanticChannelCanonicalCopy.ts` (orchestrator snapshot + shared phase table; Act observational in __DEV__ only). Pathway registry is closed in `actDescriptorTypes.ts`. Further consumers require a separate adoption audit.
