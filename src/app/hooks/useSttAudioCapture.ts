...
function buildPreservedFilename(sourceFilename: string, recordingSessionId?: string): string {
...
function preserveDebugCapture(
  ...
  const documentUri = typeof Paths.document?.uri === 'string' ? Paths.document.uri.replace(/\/+$/, '') : null;
  try {
    if (!documentUri) {
      throw new Error('Document directory URI unavailable');
    }
    const debugDirUri = `${documentUri}/debug-stt-captures`;
    const preservedFileUri = `${debugDirUri}/${buildPreservedFilename(sourceFilename, recordingSessionId)}`;
...
export function useSttAudioCapture() {
  ...
  const beginCapture = useCallback(async (recordingSessionId?: string): Promise<boolean> => {
    try {
  ...
  const endCapture = useCallback(async (recordingSessionId?: string): Promise<CapturedSttAudio | null> => {
    try {
      await recorder.stop();
    } catch (error) {
    ...
    try {
      ...
      if (!uri) {
        logWarn('AgentOrchestrator', 'stt audio capture missing file after stop', { recordingSessionId });
        return null;
      }
      ...
      const durationMillis =
        captureStartedAtRef.current != null ? Math.max(0, Date.now() - captureStartedAtRef.current) : 0;
      const mimeType = inferMimeType(uri);
      const debugPreservedUri = preserveDebugCapture(audioBase64, filename, recordingSessionId);
  ...
  const cancelCapture = useCallback(async (recordingSessionId?: string): Promise<void> => {
    try {
      await recorder.stop();
    } catch {
      /* ignore */
      ...
      logInfo('AgentOrchestrator', 'stt audio capture cancelled', { recordingSessionId });
    }
  }, [recorder]);
