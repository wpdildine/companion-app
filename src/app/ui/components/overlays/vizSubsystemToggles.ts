/**
 * Fine-grained visualization runtime toggles (dev/bisect).
 * Default: all subsystems on. Set __ATLAS_VIZ_SUBSYSTEMS__[key] = false to disable.
 * Use with viz runtime mode all_on for per-subsystem stall isolation.
 */
...
export function setVizSubsystem(key: VizSubsystemKey, enabled: boolean): void {
  ...
  g.__ATLAS_VIZ_SUBSYSTEMS__ =
    Object.keys(cur).length > 0 ? cur : undefined;
