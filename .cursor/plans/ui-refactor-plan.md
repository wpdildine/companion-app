...
- Only InteractionBand captures drag (optional).
- DevPanel is gated + diagnostic-only.

**UI blocks must be separate components:** You must implement these blocks as separate RN components (not DevPanel): AnswerCard, CardReferenceBlock, SelectedRulesBlock. The screen must render correctly with VisualizationSurface removed.
...
│   ├── VisualizationCanvasFallback.tsx
│   ├── ... (rest)
...
- **Current location:** `src/nodeMap/touchHandlers.ts` is correct for now (callback types + stub map). VisualizationCanvasR3F (and VisualizationCanvas) use it.
...
- **Wire 3D:** App (or root) gets theme and passes **only primitives** into Visualization: canvas background, palette arrays. VisualizationCanvasR3F and VisualizationCanvasFallback use those; starfieldData and formations accept optional palette args (arrays), never theme object. Shaders get data via uniforms from the component that holds theme, not from importing theme.
...
- App.tsx keeps state, effects, handlers; composes VisualizationCanvas + DevScreen vs UserVoiceView + dev toggle.
...
- **State validation:** `src/utils/validateVizState.ts` for `VisualizationEngineRef`; pure, no React/theme. Use in __DEV__ or tests.
- **Unit tests:** theme (getTheme keys and valid colors), viz state (createDefaultVisualizationRef, validateVizState), touch classification if extracted, optional UI snapshots.
...
4. Canvas Gesture (only if you explicitly enable a InteractionBand)
...
**Current pass:** `InteractionBand` is ON only in user mode with no panels visible (`!debugEnabled && !anyPanelVisible`), and OFF otherwise.

When `InteractionBand` is on:
