...
function copyRecurse(srcDir, destDir, excludeDirName) {
  ...
  for (const e of entries) {
    ...
    if (e.name === excludeDirName) continue;
    if (e.isDirectory()) {
...
function run() {
  ...
  copyRecurse(sourcePack, DEST, 'models');
  console.log('Synced pack (excluding models/) to', DEST);

  // Build pack_identity.json from source paths (before we might have overwritten manifest)
